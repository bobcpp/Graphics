...
projected_object = bilin.translate([0, 2, 80], copy=True).project(1, 10, 10, 1)
...
while True:
    ...
    if hold:
        bilin = bilin.rotateX(axis['x']).rotateY(axis['y']).rotateZ(axis['z'])
        projected_object = bilin.translate([0, 2, 80], copy=True).project(1, 10, 10, 1)

    screen.fill((0, 0, 0))
    projected_object.prepare(screen, copy=True).translate([0, Height // 4, 0]).draw(screen, wireframe = True)
